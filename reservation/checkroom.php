...
*/
if(isset($_SESSION['username'])){
$homepage = "<a href=\"sessiondestroy.php?unset=1\" class=\"button round blackblur fontslabo\" >1</a>";
}
else{$homepage = "<a href=\"sessiondestroy.php?unset=11\" class=\"button round blackblur fontslabo\" >1</a>";}


if(isset($_POST["checkin"]) && !empty($_POST["checkin"]) && isset($_POST["checkout"]) && !empty($_POST["checkout"])){

	/*if($_POST["checkin"] > $_POST["checkout"]){
		echo "<script>prompt(\"CheckIn date Cannot be Greater Than CheckOut Date\");</script>";
...
<title>Reservation - Select Room</title>
<meta name="copyright" content="Mac Copyright (c) 2019">
...
<div class="large-12 columns">
		...
		<?php echo"$homepage";?>
			<p class="fontgrey blackblur">Cancel Reservation</p>
		</div>
		<div class="large-3 columns centerdiv">
			<a href="#" class="button round fontslabo" style="background-color:#2ecc71;">2</a>
			<p class="fontgrey blackblur">Select Room</p>
		</div>
		<div class="large-3 columns centerdiv">
			<a href="#" class="button round blackblur fontslabo" >3</a>
			<p class="fontgrey blackblur">Guest Details</p>
		</div>
		<div class="large-3 columns centerdiv">
...
<div class="row">
	<div class="large-4 columns blackblur fontcolor" style="margin-left:-10px; padding:10px;">
		...
		<div class="large-12 columns " >
		<p><b>Your Reservation Deatail</b></p><hr class="line">
				...
				<div class="row">
				  ...
				  <!--div class="row">
					<div class="large-12 columns" >
<?php// echo"<br><a name=\"submit\" href=\"sessiondestroy.php?unset=1\" class=\"button small fontslabo\" style=\"background-color:#2ecc71; width:100%;\" >Edit Reservation</a>"; ?>
					</div>
				  </div-->
				</form>
		</div>
...
$res = mysqli_query($dbhandle, "select * from roomnumbers where room_id = ".$row['room_id']."");
			echo mysqli_error($dbhandle);
			if(mysqli_num_rows($res) > 0){
			  while($row0 = mysqli_fetch_array($res)){
			  	$_SESSION['guestform'] = "guestform";
			//	$rmavailble = json_decode($row0['available_rm'],true);
			  	$rmavailble = $row0['total_room'] - $row0['total_occupied'];
				if($rmavailble>0){
					$noroom = 203;
				// $totalrm = count($rmavailble);
				// if($totalrm>=0){ //Only display rooms that are available
		...
		print "						<br><span class=\"fontgrey\">Location : </span> ".$sub_row2['location']."</p>\n";
		print "\n";
		print "						</div>\n";
		print "						<div class=\"large-4 columns\">\n";
		print "						<p ><span class=\"fontgrey\">Rate : GHS </span><span style=\"font-size:24px;\">".$sub_row2['rate']."</span><span class=\"fontgrey\">/ night</span><br>\n";
		print "						<span style=\"text-align:right;\">".$rmavailble." room(s) available</span>\n";
		print "						</p>\n";
		print "							<div class=\"row\">\n";
		print "								<div class=\"large-11 columns\">\n";
		print "									<label class=\"fontcolor\">\n";
...
if($noroom==101){echo "<p><b>Sorry!! No Room Available</b></p><hr>";}	
//else{echo "<p><b>No room available</b></p><hr>";}
